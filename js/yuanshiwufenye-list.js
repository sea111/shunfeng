$(function(){
	...
	//临时加密方法 wpEncrypt(key,params),params是要加密的请求参数，msg是请求加密时获取的数据，codeValue=msg.data.result是加密的值。
			WPBridge.callMethod("JsInvokeNative","wpEncrypt",{key:key,params:[brandId,1,20]},function(msg){
					...
					success:function(mes){
						if(mes.errMsg == "AES加密解密失败"){
							...
							function ajaxCallback(inx){
								var i=inx+1;
								if(mess[i]){
									ajax(key,mess[i].categoryId,mess[i],i,ajaxCallback);
								}
							}
							ajax(key,mess[0].categoryId,mess[0],0,ajaxCallback);
						}
					}
				})
			})
	...
	function ajax(key,catId,cateArr,j,fn){
		WPBridge.callMethod("JsInvokeNative","wpEncrypt",{key:key,params:[brandId,1,2,catId]},function(msg){codeValue=msg.data.result;
		$.ajax({
			type:"post",
				...
				if(datas.errMsg == "AES加密解密失败"){			
					...
					if(datas.success){
						...
						var mes=datas.data.list;
							str+='<ul class="ul">';
							str+="<p class='backG'></p>"
							str+='<li class="li listTitle" resultName="'+cateArr.categoryName+'" resultid="'+cateArr.categoryId+'"><i class="bgLift">'+cateArr.categoryName+'</i>'							
							if(mes.length>1){
								str+='<span resultName="'+cateArr.categoryName+'" resultid='+cateArr.categoryId+' class="more">更多</span><i resultName="'+cateArr.categoryName+'" resultid='+cateArr.categoryId+' class="bgRight"></i>'
							}
							str+='</li>'
